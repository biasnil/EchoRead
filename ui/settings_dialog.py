...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings: SettingsManager, cache: VoiceCache, catalog: VoiceCatalog, profile: ProfileManager,
        ...
        lay.addSpacing(4)
        lay.addWidget(make_label("Reading PDFs and scans", "cardtitle"))
        self._pdf_headings = QCheckBox("Read headings — titles and section headings")
        self._pdf_headings.setChecked(settings.pdf_headings)
        self._pdf_footnotes = QCheckBox("Read footnotes — the notes at the foot of a page")
        self._pdf_footnotes.setChecked(settings.pdf_footnotes)
        self._pdf_furniture = QCheckBox("Read page headers, footers and page numbers")
        self._pdf_furniture.setChecked(settings.pdf_furniture)
        self._pdf_tables = QCheckBox("Read tables row by row — \"Name: Ana. Age: 30.\"")
        self._pdf_tables.setChecked(settings.pdf_tables)
        self._pdf_pictures = QCheckBox("Show pictures in the reader")
        self._pdf_pictures.setChecked(settings.pdf_show_pictures)
        self._pdf_images = QCheckBox("Read the words inside pictures")
        self._pdf_images.setChecked(settings.pdf_read_images)
        self._read_math = QCheckBox("Say formulas out loud")
        self._read_math.setChecked(settings.read_math)
        hints = {self._pdf_pictures: "Charts, photos and diagrams stay between the paragraphs. They are never read aloud.",
                 self._pdf_images: "Off by default. Reads a figure's labels, or OCRs a picture that has no text of its own (slower).",
                 self._read_math: "\"x² + y² = z²\" becomes \"x squared plus y squared equals z squared\"."}
        for box in (self._pdf_headings, self._pdf_footnotes, self._pdf_furniture, self._pdf_tables, self._pdf_pictures, self._pdf_images, self._read_math):
            lay.addWidget(box)
            if box in hints:  # a checkbox can't wrap its text, so the details go in a label under it
                hint = make_label(hints[box], "muted", wrap=True)
                hint.setContentsMargins(26, 0, 0, 0)
                lay.addWidget(hint)
        self._ocr_smart = QCheckBox("Smart layout for scans")
        self._ocr_smart.setChecked(settings.ocr_smart)
        lay.addWidget(self._ocr_smart)
        smart_hint = make_label("Finds columns, headers, footnotes and tables on scanned pages. Slower, and it downloads extra models the first time.",
                                "muted", wrap=True)
        smart_hint.setContentsMargins(26, 0, 0, 0)
        lay.addWidget(smart_hint)
        form2 = QFormLayout()
        form2.setSpacing(10)
        self._pdf_verses = QComboBox()
        self._pdf_verses.addItem("Automatic — Bibles: one verse per paragraph", "auto")
        self._pdf_verses.addItem("Always — one paragraph per verse, numbers not read", "always")
        self._pdf_verses.addItem("Never — leave numbers in the text", "never")
        self._pdf_verses.setCurrentIndex(max(self._pdf_verses.findData(settings.pdf_verses), 0))
        self._ocr_quality = QComboBox()
        self._ocr_quality.addItem("Standard", "standard")
        self._ocr_quality.addItem("Sharp — slower, better for small print in scans", "sharp")
        self._ocr_quality.setCurrentIndex(0 if settings.ocr_quality == "standard" else 1)
        for combo in (self._pdf_verses, self._ocr_quality):  # a long item must not make the whole dialog wider than the window
            combo.setSizeAdjustPolicy(QComboBox.SizeAdjustPolicy.AdjustToMinimumContentsLengthWithIcon)
            combo.setMinimumContentsLength(18)
        form2.addRow("Verse numbers", self._pdf_verses)
        form2.addRow("OCR sharpness", self._ocr_quality)
        lay.addLayout(form2)
        lay.addWidget(make_label("Columns are always read one after the other. These choices apply to files you open from now on, "
                                 "or after Regions → Re-run text extraction.", "muted", wrap=True))
    ...
    def _save(self) -> None:
        ...
        s.pdf_headings = self._pdf_headings.isChecked()
        s.pdf_footnotes = self._pdf_footnotes.isChecked()
        s.pdf_furniture = self._pdf_furniture.isChecked()
        s.pdf_tables = self._pdf_tables.isChecked()
        s.pdf_show_pictures = self._pdf_pictures.isChecked()
        s.pdf_read_images = self._pdf_images.isChecked()
        s.read_math = self._read_math.isChecked()
        s.pdf_verses = self._pdf_verses.currentData()
        s.ocr_quality = self._ocr_quality.currentData()
        s.ocr_smart = self._ocr_smart.isChecked()
